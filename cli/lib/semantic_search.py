...
class ChunkedSemanticSearch(SemanticSearch):
    ...
    def search_chunks(self, query: str, limit: int = 10) -> list[SearchResult]:
        ...
        for chunk_score in chunk_scores:
            ...
            if (
                movie_idx not in best_scores
                or chunk_score["score"] > best_scores[movie_idx]
            ):
                best_scores[movie_idx] = chunk_score["score"]

        sorted_best_scores = sorted(
            best_scores.items(), key=lambda x: x[1], reverse=True
        )
        top_scores = sorted_best_scores[:limit]
